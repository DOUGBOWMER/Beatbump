...
const alphatrack_config_path = path.resolve(root, "alphatrack.conf.json");
const json_config = JSON.parse(fs.readFileSync(alphatrack_config_path, { encoding: "utf8" }));
...
const cli = new CLI("AlphaTrack CLI");

cli
	.command("build", [""])
	.describe("Builds AlphaTrack")
	.action(() => {
		try {
			if (!alphatrack_config_path) {
				console.error(`No config file was found. Please create a file named 'alphatrack.config.json' at ${rootPath}`);
				return;
			}
			...
			console.info(`Building AlphaTrack, please wait...`);
